...
export default function UsersPage() {
  ...
  return (
    <>
      <PageHeader title="Users" description="Create driver or admin logins for the test workspace." />
      <div className="grid gap-4 lg:grid-cols-[420px_1fr]">
        <form onSubmit={submit} className="rounded border border-kp-line bg-white p-4 shadow-panel">
          <h2 className="mb-3 text-lg font-bold text-kp-ink">Add Login</h2>
        ...
        <section className="rounded border border-kp-line bg-white shadow-panel">
          <div className="border-b border-kp-line p-3">
            <h2 className="font-bold text-kp-ink">Current Logins</h2>
          </div>
          ...
          <div className="mt-3 grid gap-3 md:grid-cols-3">
            {drivers.length > 0 ? drivers.map((driver) => (
              <article key={driver.id} className="rounded border border-kp-line bg-kp-paper p-3">
                ...
                <p className="mt-1 text-sm font-semibold text-kp-green">{driver.availabilityStatus ?? "Unavailable"}</p>
                <p className="mt-2 text-sm text-stone-600">{driver.availabilityNotes || "No availability notes."}</p>
              </article>
